...
import { ChevronDown, ChevronUp, BookOpen, User, Calendar, ArrowRight } from 'lucide-react';
  ...
  const fetchLectures = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // 講義が1件だけ（時間割連携等）の場合は自動でアコーディオンを展開
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-text-sub mt-1">
          慶應の開講シラバスから教科書と、現在出品されている中古教科書をAND検索できます。
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* 左ペイン: シラバス検索パネル (4カラム) */}
        <form onSubmit={handleSearch} className="lg:col-span-4 card-premium p-6 space-y-5 bg-surface sticky top-20">
          <div className="flex justify-between items-center border-b border-border-main pb-3">
            <h3 className="font-bold text-sm text-text-main">検索条件</h3>
            <button
              type="button"
              onClick={resetFilters}
              className="text-xs text-text-sub hover:text-keio-navy transition font-medium"
            >
              条件をリセット
            </button>
          </div>
          ...
          <div className="space-y-4 text-xs">
            {/* キーワード・授業名 */}
            <div>
              <label className="block font-bold text-text-sub mb-1.5">キーワード</label>
              <input
                type="text"
                placeholder="授業名、教員名、教科書名など..."
            ...
            <div>
              <button
                ...
                className="flex items-center gap-1 text-[11px] font-bold text-keio-navy hover:underline transition"
              >
                {showAdvanced ? (
                  <>詳細検索を閉じる <ChevronUp className="h-3.5 w-3.5" /></>
                ) : (
            ...
            <AnimatePresence>
              {showAdvanced && (
                ...
                >
                  {/* 授業形態 */}
                  <div>
                    <label className="block font-bold text-text-sub mb-1.5">授業形態 (実施形態)</label>
                    <select
                      className="w-full px-3 py-2 bg-background border border-border-main rounded-lg text-text-main focus:outline-none focus:border-keio-navy"
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex justify-between items-center text-xs text-text-sub">
            <span>検索結果: {lectures.length} 件</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="card-premium p-12 text-center text-text-sub text-sm">
              条件に一致する講義が見つかりませんでした。条件を変更して再検索してください。
            </div>
          ) : (
            <div className="space-y-4">
              {lectures.map((lec) => {
                const isExpanded = expandedLectureId === lec.id;
                // 使用教科書に紐づくAVAILABLEなItemの総数をカウント
                  ...
                  >
                    ...
                    >
                      <div className="space-y-1.5">
                        ...
                        <h4 className="font-bold text-text-main text-base">{lec.name}</h4>
                      ...
                      <div className="flex items-center gap-4 shrink-0 text-xs">
                        <div className="text-right">
                          <div className="font-semibold text-text-main">教科書: {lec.textbooks?.length || 0}冊</div>
                          <div className="text-[10px] text-text-sub mt-0.5">出品中の中古: {totalListings}件</div>
                        </div>
                        <ChevronDown
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        ...
                        >
                          <h5 className="text-xs font-bold text-text-sub uppercase tracking-wider">
                            📖 シラバス指定教科書・教材一覧
                          </h5>

                          {(!lec.textbooks || lec.textbooks.length === 0) ? (
                            <p className="text-xs text-text-sub">指定された教科書はありません。</p>
                          ) : (
                            <div className="space-y-4">
                              {lec.textbooks.map((tb) => (
                                <div key={tb.id} className="p-4 bg-surface border border-border-main rounded-xl space-y-3 shadow-sm">
                                  ...
                                  <div className="pt-3 border-t border-border-main/50 space-y-2">
                                    <div className="text-[11px] font-bold text-text-sub">出品中の中古本 ({tb.items?.length || 0}件)</div>
